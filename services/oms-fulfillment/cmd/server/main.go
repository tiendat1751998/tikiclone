...
import (
	...
	"github.com/tikiclone/tiki/services/oms-fulfillment/internal/health"
	"github.com/tikiclone/tiki/services/oms-fulfillment/internal/infrastructure/kafka"
	"github.com/tikiclone/tiki/services/oms-fulfillment/internal/infrastructure/mysql"
	redisinfra "github.com/tikiclone/tiki/services/oms-fulfillment/internal/infrastructure/redis"
	"github.com/tikiclone/tiki/services/oms-fulfillment/internal/tracing"
	httptransport "github.com/tikiclone/tiki/services/oms-fulfillment/internal/transport/http"
...
func main() {
	...
	repo := mysql.NewFulfillmentRepository(db)
	...
	svc := application.NewFulfillmentService(repo, redisStore, kafkaProducer, cfg.Fulfillment.DefaultCarrier)

	var consumer *kafka.Consumer
	if len(cfg.Kafka.Brokers) > 0 && cfg.Kafka.Brokers[0] != "" {
		topics := []string{cfg.Kafka.TopicPrefix + ".events"}
		consumer = kafka.NewConsumer(cfg.Kafka, topics, kafka.NewFulfillmentEventHandler())
	}
	...
	go func() {
		logger.Info("starting oms-fulfillment service",
	...
	if consumer != nil {
		...
		go func() {
			...
			defer func() {
				if r := recover(); r != nil {
					zap.L().Error("panic in kafka consumer", zap.Any("recover", r))
				}
			}()
	...
	go func() {
		...
		defer func() {
			if r := recover(); r != nil {
				zap.L().Error("panic in outbox worker", zap.Any("recover", r))
			}
		}()
		...
		for {
			...
			case <-ticker.C:
				func() {
					...
					defer oCancel()
					if err := svc.ProcessOutboxEvents(oCtx); err != nil {
	...
	<-ctx.Done()
	logger.Info("shutting down oms-fulfillment service...")
	...
	logger.Info("oms-fulfillment service stopped")
}
